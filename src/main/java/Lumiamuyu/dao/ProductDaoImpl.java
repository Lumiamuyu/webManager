...
/*    @Override
    public List<Product> getLists() {
        return JDBCUtil.executeQuery("select * from brand,product where product.brand_id=brand.brand_id", new RowMap<Product>() {
            @Override
            public Product RowMapping(ResultSet resultSet) {
                Product p = new Product();
                try {
                    ...
                    Brand b = new Brand();
                    b.setBrandId(resultSet.getInt("brand_id"));
                    b.setBrandName(resultSet.getString("brand_name"));
                    b.setBrandDes(resultSet.getString("brand_des"));
                    set.add(b);
                    p.setSet(set);
                } catch (SQLException e) {
                    e.printStackTrace();
    ...
    public int add(Product product) {
        return JDBCUtil.executeUpdate("insert into product(product_name,price,product_des,url,reverse,brand_id) values(?,?,?,?,?,?)",product.getProductName(),product.getPrice(),product.getProductDes(),product.getUrl(),product.getReverse(),product.getBrandId());
    }
    ...
    public int update(Product product) {
        return JDBCUtil.executeUpdate("update product set product_name=?,product_des=?,url=?,price=?,reverse=?,brand_id=? where product_id=?",product.getProductName(),product.getProductDes(),product.getUrl(),product.getPrice(),product.getReverse(),product.getBrandId(),product.getProductId());
    }
